...
console.log('db', db)
console.log('exec', await db.exec({ sql: `create table if not exists t1(a, b)` }))
console.log('exec', await db.exec({ sql: `delete from t1` }))
console.log('exec', await db.exec({ sql: `insert into t1 values(1, 'hello'), (2, 'world')` }))
console.log('exec', await db.exec({ sql: "select a, b from t1", rowMode: 'object' }))
